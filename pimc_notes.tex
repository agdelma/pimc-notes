...
%that are depicted schematically in Figure~\ref{fig:pimcMoves}.
...
slices $\alpha$ and $\alpha+m$ where $m < M$. It utilizes the fact that the
free particle density matrix in \eqref{eq:rho0} is Gaussian and thus obeys a
convolution relation.  The problem is as follows:  given two known spatial
positions (beads) on worldline $i$, say $r_{i,\alpha}$ and $r_{i,\alpha+m}$,
how do we exactly sample the product of free particle density matrices:
%
\begin{equation}
    \rho_0(\vec{r}_{i,\alpha},\vec{r}_{i,\alpha+1};\tau) \cdots
    \rho_0(\vec{r}_{i,\alpha+m-1},\vec{r}_{i,\alpha+m};\tau)?
\end{equation}
%
...
%
\begin{align}
    \pi_0(\vec{r}_\gamma| \vec{r}_{\alpha},\vec{r}_{\alpha+m}) &=
    \rho_0(\vec{r}_\alpha,\vec{r}_\gamma;(\gamma-\alpha)\tau) 
\rho_0(\vec{r}_\gamma,\vec{r}_{\alpha+m};(\alpha+m-\gamma)\tau) \nonumber \\
&\propto 
\exp\left[-\frac{|\vec{r}_\gamma-\vec{r}_{\alpha}|^2}{4\lambda (\gamma-\alpha)\tau} \right]
\exp\left[-\frac{|\vec{r}_{\alpha+m}-\vec{r}_{\gamma}|^2}{4\lambda (\alpha+m-\gamma)\tau} \right]
\nonumber \\
...
%
\begin{align}
    \overline{\vec{r}}_\gamma &= \frac{1}{m}\left[(\alpha+m-\gamma)\vec{r}_{\alpha} +
(\gamma-\alpha)\vec{r}_{\alpha+m}\right] \nonumber \\
\sigma^2 &= \frac{1}{\frac{1}{(\alpha + m -\gamma)\tau} +
\frac{1}{(\gamma-\alpha)\tau}}.
\end{align}
%
As \eqref{eq:pi0} is a simple Gaussian with mean $\overline{x}_\gamma$ and
...
\begin{enumerate}
    ...
    \item Choose a random integer $\alpha \in [0,M-1]$ such that the start of
        ...
        $\vec{r}_{i,\alpha+\overline{m}}$, where $\overline{m}$ is an algorithm
        parameter that can be optimized. These beads are held fixed during the update.
    \item Using $\vec{r}_{i,\alpha}$ and $\vec{r}_{i,\alpha+\overline{m}}$ construct the
...
%
\end{enumerate}
Such a move should enjoy a reasonably high acceptance rate.
...
\end{equation}
%
where $\mu$ is the grand canonical chemical potential,
$g(\vec{r},\vec{r'};\tau-\tau')$ is (up to a normalization constant)
equal to the single particle Matsubara Green function
%
\begin{equation}
g(\vec{r},\vec{r'};\tau-\tau') \equiv \mathcal{Z}\left \langle 
\hat{\mathsf{T}}_\tau \hat{\psi}(\vec{r},\tau)\hat{\psi}^\dagger(\vec{r}',\tau')
\right \rangle
\end{equation}
%
and $\hat{\mathsf{T}}_\tau$ is the imaginary time ordering operator. Working
in this extended ensemble is trivial, as we just allow for configurations with
open worldlines.  Such configurations are unphysical however, and we only make
measurements when there are no worms present in the simulation. We refer to the 
...
%~~~~~~~~~~~~~~~~~~~~~~~~~
An \emph{open} update moves the ensemble from diagonal to off-diagonal, and can
only be attempted when there are no worms present.  It proceeds as follows:
...
\noindent
The \emph{close} move provides a detailed balance compliment to \emph{open} and
...
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~
An \emph{insert} move involves the addition to the ensemble of an entire worm
and moves the configuration from diagonal to off-diagonal.  It is only possible
if there are no worms present in the system and proceeds as follows:
\begin{enumerate}
...
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
The \emph{advance} and \emph{recede} updates do not change the nature of the
...
\subsubsection{Swap}
%~~~~~~~~~~~~~~~~~~~
The \emph{swap} update is behind the huge speedup inherent in the worm
algorithm, as it allows us to sample different topological winding sectors
(permutations) using only local updates.  It does not, therefore, suffer from
the horrendous $N!$ scaling of brute-force permutation sampling. It only
operates on off-diagonal configurations, in the presence of a worm, and does
not change the ensemble. It provides a method by which a single worm grows by
$n M$ time slices where $1 \le n \le N$ by ``swapping'' its head with a
proximate unbroken particle worldline.  To perform a swap move:
\begin{enumerate}
    \item Determine the time slice $\alpha_h$ and spatial location
        $\vec{r}_{i,\alpha_h}$ of the worm head.
